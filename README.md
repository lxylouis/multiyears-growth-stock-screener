...
**核心逻辑**：对指数内每只股票，计算 `window_years` 年滚动增长倍率，筛选出持续成长的优质标的，结合股价与营收双维度交叉验证。
...
| 参数 | 默认值 | 说明 |
|:---|---:|:---|
| `window_years` | 3 | 每个窗口跨越的年数 |
| `num_windows` | 5 | 从基准年往过去推的窗口数量 |
| `cond_a_min` (条件A) | 1.3 | 所有窗口倍率均需 ≥ 此值（≈9.1% CAGR/窗口） |
| `cond_b_threshold` (条件B) | 1.5 | 最新窗口倍率需 > 此值（≈14.5% CAGR） |
...
| `--window-years` | 从YAML读取 | 每个窗口跨 `window_years` 年（如5=5年增长倍率，3=3年） |
| `--num-windows` | 从YAML读取 | 从基准年往过去推 `num_windows` 个窗口（如3=3个 `window_years` 年窗口） |
| `--price-base-yyyymm` | YYYY-MM | 股价基准年月（如 `2026-03`），默认今年3月 |
| `--revenue-base-yyyy` | 年份 | 营收基准年（如 `2026`），默认A股=今年，美股/港股=去年 |

> **时间窗口自动推算示例**：`--window-years 3 --num-windows 5 --price-base-yyyymm 2026-03`
> → 自动生成 5 个窗口：2026/2023, 2025/2022, 2024/2021, 2023/2020, 2022/2019（`window_years`=`3`, `num_windows`=`5`）
...
**期数**：`num_windows` 期，每期跨 `window_years` 年（默认值，可通过 YAML 或 CLI 调整）。

```text
 2026/3末  ÷  2023/3末  =  第1期 增长倍率  ← 最新
 2025/3末  ÷  2022/3末  =  第2期 增长倍率
 2024/3末  ÷  2021/3末  =  第3期 增长倍率
 2023/3末  ÷  2020/3末  =  第4期 增长倍率
 2022/3末  ÷  2019/3末  =  第5期 增长倍率  ← 最远
```
...
**期数**：`num_windows` 期，每期跨 `window_years` 年（默认值）。

```text
 2025年报  ÷  2022年报  =  第1期 营收增长倍率  ← 最新
 2024年报  ÷  2021年报  =  第2期 营收增长倍率
 2023年报  ÷  2020年报  =  第3期 营收增长倍率
 2022年报  ÷  2019年报  =  第4期 营收增长倍率
 2021年报  ÷  2018年报  =  第5期 营收增长倍率  ← 最远
```
...
# 时间窗口
defaults:
  window_years: 3          # 每个窗口跨 3 年
  num_windows: 5           # 5 个窗口

# 筛选阈值（可选，不填则使用 YAML screening 节配置）
screening:
  cond_a_min: 1.3
  cond_b_threshold: 1.5

# 显式时间窗口（如不填，由 defaults 自动生成）
periods:
  price: [[2026,2023],[2025,2022],[2024,2021],[2023,2020],[2022,2019]]
  revenue: [[2025,2022],[2024,2021],[2023,2020],[2022,2019],[2021,2018]]
  labels: ['26/23','25/22','24/21','23/20','22/19']
```

每对 `[终点年份, 起点年份]` 定义一期 `window_years` 年滚动增长倍率。不填 `periods` 时自动从 `defaults` 推算。`screening` 节控制保留条件的两个阈值（条件A = 全期≥ `cond_a_min`，条件B = 最新期 > `cond_b_threshold`）。
...
| 条件 | 规则 | 含义 | 默认值 |
|:---|:---|---:|---:|
| **A** | 所有窗口倍率 **均 ≥ `cond_a_min`** | 历史上没有过大幅衰退 | **`cond_a_min`** |
| **B** | 最新窗口倍率 **> `cond_b_threshold`** | 最近 `window_years` 年增长超过 `cond_b_threshold` 倍（≈14.5% CAGR），跑赢大盘 | **`cond_b_threshold`** |

**为什么是这个规则？**
- 条件 A 过滤掉了"曾经好过但后来不行了"的标的
- 条件 B 要求**最近 `window_years` 年持续强劲**，年均 14.5% 以上——高于大多数市场的长期平均回报

**默认窗口：`window_years`年×`num_windows`期**（最近 `num_windows` 个 `window_years` 年窗口，权重线性递减 1:2:...:`num_windows`）
- 两者并集 = 持续成长型 + 新晋成长型

> 阈值可通过各指数 YAML 配置中 `screening` 节的 `cond_a_min` 和 `cond_b_threshold` 调整，默认值通过 YAML `screening` 节配置。
...
```python
# `num_windows`期权重示例（默认 `num_windows`=5）
